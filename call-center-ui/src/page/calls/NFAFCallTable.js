...
function NFAFCallTable({reload, user_data, textConstants, getUser}) {
    const columns = [
        {
    ...
    const getCallsList = (page, pageSize) => {
        setLoading(true);
        axios.get(`${apiUrl}/call-center/call/getAFNFCallsByOperatorId?operator_id=${user_data.login}&is_history=0&page=${page}&size=${pageSize}`).then(res => {
    ...
    return (
        <>
            ...
            {visibleModal && currentItem && (
                <Registration inf2={currentItem.inf2} visible={visibleModal} setvisible={setVisibleModal}
                              item={currentItem}/>)}
        </>
    );
}
