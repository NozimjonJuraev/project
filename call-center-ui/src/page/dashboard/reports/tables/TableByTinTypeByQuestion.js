...
function TableByTinTypeByQuestion({lang, report_filter, setReportFilter, textConstants}) {
    const [tableData, setTableData] = useState([{}]);
    const [regionId, setRegionId] = useState(0);
    const [isLoadingXlsByTinType, setIsLoadingXlsByTinType] = useState(false);
    ...
    const getTerritory = () => {
        if (!Object.keys(report_filter).length == 0) {
            if (!report_filter.regionId > 0) {
                axios.post(`${apiUrl}/call-center/reports/report-by-type-tin-by-questions`, {
    ...
    const columns = [
        ...
        {
            title: textConstants.APPLICANT_TYPE,
            key: 'id',
    ...
    const downloadAsExcelByTinType = () => {
        setIsLoadingXlsByTinType(true);
        axios.post(`${apiUrl}/call-center/reports/report-xls-by-type-tin-by-questions`, {
                regionId: report_filter.regionId,
                date: report_filter.date,
                callCenterTypeId: report_filter.callCenterTypeId
            },
            ...
            if (res.status === 200) {
                ...
                link.href = url;
                link.setAttribute('download', "распределение вопросы исходили из какой категории_".replace(" ", "_") + moment(new Date()).format("DD.MM.YYYY_HH-mm-ss") + ".xlsx");
                document.body.appendChild(link);
                link.click();
            } else {
                message.warning(textConstants.RETRIEVE_DATA_ERROR);
            }
            setIsLoadingXlsByTinType(false);
        }).catch(err => {
            setIsLoadingXlsByTinType(false);
            console.log(err);
            message.warning(textConstants.RETRIEVE_DATA_ERROR);
                    ...
                    <Row style={{width: "100%"}}>
                        <Col xs={24} md={12}>
                            <h2 style={{
                                textAlign: "left",
                                fontSize: 18
                            }}>{textConstants.REPORT_DISTRIBUTE_QUESTIONS_BY_TIN_TYPES.charAt(0).toUpperCase() + textConstants.REPORT_DISTRIBUTE_QUESTIONS_BY_TIN_TYPES.slice(1)}</h2>
                        </Col>
                        <Col xs={24} md={12}>
                            <Button type="primary" size="small" style={{float: "right"}}
                                    onClick={downloadAsExcelByTinType}
                                    loading={isLoadingXlsByTinType}>
