...
function ApplicantsByTheme({isVisible, setIsVisible, item, dashboard_filter, textConstants}) {
    const columns = [
        {
    ...
    useEffect(() => {
        if (!Object.keys(dashboard_filter).length == 0 && item.themeId != undefined) {
            setLoading(true);
            axios.post(`${apiUrl}/call-center/dashboard/getRegisteredApplicantsByTheme`, {
    ...
    return (
        <Modal
            class="applicants-by-theme"
            title={firstToUpper(textConstants.BY_CATEGORY)}
            visible={isVisible}
            width={1660}
